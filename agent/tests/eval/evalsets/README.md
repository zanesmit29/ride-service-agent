...
12. Used uv run --active adk eval ./app "tests/eval/evalsets/memory_regression.evalset.json" --config_file_path="tests/eval/eval_config.json" for example to run specific evalsets --> Always run from the .\agent directory either in PowerShell or Bash to ensure the correct relative paths.
